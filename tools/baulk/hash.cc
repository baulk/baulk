...
bool sha256sum(std::wstring_view file, std::wstring &hv, bela::error_code &ec) {
  ...
  for (;;) {
    DWORD dwread = 0;
    if (ReadFile(FileHandle, buffer, sizeof(buffer), &dwread, nullptr) != TRUE) {
      ec = bela::make_system_error_code();
      return false;
    }
    ctx.Update(reinterpret_cast<const uint8_t *>(buffer), static_cast<size_t>(dwread));
...
bool HashEqual(std::wstring_view file, std::wstring_view hashvalue, bela::error_code &ec) {
  ...
  if (pos == std::wstring_view::npos) {
    ...
    ec = bela::make_error_code(1, L"checksum mismatch expected ", hashvalue, L" actual ", nv);
    return false;
  }
  ...
  for (const auto &h : hv) {
    ...
    ec = bela::make_error_code(1, L"checksum mismatch expected ", hash, L" actual ", hv);
    return false;
  }
