...
New-ItemProperty -Path 'Registry::HKEY_CLASSES_ROOT\Directory\shell\baulk\command' -Name '(Default)' -PropertyType String -Value "`"$BaulkTerminal`" --vs" | Out-Null
...
New-ItemProperty -Path 'Registry::HKEY_CLASSES_ROOT\Directory\Background\shell\baulk\command' -Name '(Default)' -PropertyType String -Value "`"$BaulkTerminal`" --vs" | Out-Null
