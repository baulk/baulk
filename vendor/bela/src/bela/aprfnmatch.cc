...
#include <cctype>
// https://github.com/apache/apr/blob/trunk/strings/apr_fnmatch.c
namespace bela {
...
static inline int fnmatch_ch(const char **pattern, const char **string, int flags) {
  ...
  if (**pattern == '[') {
    ...
    leadingclosebrace:
      ...
      if (((*pattern)[1] == '-') && ((*pattern)[2] != ']')) {
        ...
        *pattern += (escape && ((*pattern)[2] == '\\')) ? 3 : 2;
        ...
        if ((**string >= *startch) && (**string <= **pattern))
          result = 0;
        else if (nocase && (isupper(**string) || isupper(*startch) || isupper(**pattern)) &&
                 (tolower(**string) >= tolower(*startch)) && (tolower(**string) <= tolower(**pattern)))
          result = 0;
      ...
      if ((**string == **pattern))
        result = 0;
      else if (nocase && (isupper(**string) || isupper(**pattern)) && (tolower(**string) == tolower(**pattern)))
        result = 0;
...
int apr_fnmatch(const char *pattern, const char *string, int flags) {
  ...
  firstsegment:
     ...
     */
    while (*pattern) {
      if ((string > strendseg) || ((string == strendseg) && (*pattern != '*')))
        break;

      if (slash && ((*pattern == '/') || (escape && (*pattern == '\\') && (pattern[1] == '/'))))
        break;
      ...
      if (wild) {
        ...
        for (matchptr = pattern, matchlen = 0; 1; ++matchlen) {
          if ((*matchptr == '\0') ||
              (slash && ((*matchptr == '/') || (escape && (*matchptr == '\\') && (matchptr[1] == '/'))))) {
      ...
      while (*pattern && (string < strendseg)) {
        ...
        if (slash && ((*string == '/') || (*pattern == '/') || (escape && (*pattern == '\\') && (pattern[1] == '/'))))
          break;
    ...
    if (*pattern && !(slash && ((*pattern == '/') || (escape && (*pattern == '\\') && (pattern[1] == '/')))))
      return APR_FNM_NOMATCH;
  }
