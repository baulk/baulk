...
public:
  ...
  bool GetTranslationId(LPVOID lpData, UINT unBlockSize, WORD wLangId,
    ...
    for (lpwData = (LPWORD)lpData;
         (LPBYTE)lpwData < ((LPBYTE)lpData) + unBlockSize; lpwData += 2) {
      if (*lpwData == wLangId) {
        dwId = *((DWORD *)lpwData);
        return true;
      }
  ...
  bool VerQuery(VersionInfo &vi, bela::error_code &ec) {
    ...
    DWORD dwLangCode = 0;
    if (!GetTranslationId(info, ilen, GetUserDefaultLangID(), dwLangCode,
                          FALSE)) {
      if (!GetTranslationId(info, ilen, GetUserDefaultLangID(), dwLangCode,
                            TRUE)) {
        if (!GetTranslationId(info, ilen,
    ...
    auto block = bela::StringCat(
        L"\\StringFileInfo\\",
        bela::AlphaNum(bela::Hex(dwLangCode & 0x0000FFFF, bela::kZeroPad4)),
        bela::AlphaNum(
            bela::Hex((dwLangCode & 0xFFFF0000) >> 16, bela::kZeroPad4)),
        L"\\");
