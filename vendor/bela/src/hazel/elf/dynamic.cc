...
bool File::DynString(int tag, std::vector<std::string> &sv, bela::error_code &ec) {
  ...
  if (fh.Class == ELFCLASS32) {
    while (dss.size() >= 8) {
      auto t = EndianCastPtr<uint32_t>(dss.data());
      auto v = EndianCastPtr<uint32_t>(dss.data() + 4);
      dss.remove_prefix(8);
      if (t == tag) {
  ...
  while (dss.size() >= 16) {
    auto t = EndianCastPtr<uint64_t>(dss.data());
    auto v = EndianCastPtr<uint64_t>(dss.data() + 8);
