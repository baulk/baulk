...
static constexpr std::array<UnicodeRange, 295> s_wideAndAmbiguousTable{
    ...
    UnicodeRange{0xfe20, 0xfe2f, CodepointWidth::Narrow}, // narrow combining ligatures (split into left/right halves,
                                                          // which take 2 columns together)
...
CodepointWidth GetQuickCharWidth(const char32_t u) noexcept {
  if (0x20 <= u && u <= 0x7e) {
    /* ASCII */
...
CodepointWidth GetCharWidth(const char32_t u) noexcept {
  const auto it = std::lower_bound(s_wideAndAmbiguousTable.begin(), s_wideAndAmbiguousTable.end(), u);
  ...
  if (it != s_wideAndAmbiguousTable.end() && u >= it->lowerBound && u <= it->upperBound) {
    return it->width;
  }
