// ---------------------------------------------------------------------------
// Copyright © 2026, Bela contributors
//
...
// limitations under the License.
// ---------------------------------------------------------------------------
#ifndef BELA_STR_SPLIT_NARROW_HPP
#define BELA_STR_SPLIT_NARROW_HPP
#pragma once
#include <algorithm>
#include <cstddef>
#include <map>
...
#include <vector>
#include "ascii.hpp"
#include "__strings/str_split_narrow_internal.hpp"
...
// (`const char*`, `std::string`, or `std::string_view`) is passed in place
...
//
//   std::vector<std::string> v1 = bela::StrSplit(L"a, b, c", L", ");
//
//   using bela::ByString;
//   std::vector<std::string> v2 = bela::StrSplit(L"a, b, c",
//                                                ByString(L", "));
...
//   // the following two splits are equivalent.
//   std::vector<std::string> v1 = bela::StrSplit(L"a,b,c", ',');
//   using bela::ByChar;
//   std::vector<std::string> v2 = bela::StrSplit(L"a,b,c", ByChar(','));
//   // v[0] == L"a", v[1] == L"b", v[2] == L"c"
//
...
// equivalent:
//
//   std::vector<std::string> v = bela::StrSplit(L"a-b", '-');
//
//   using bela::ByChar;
//   std::vector<std::string> v = bela::StrSplit(L"a-b", ByChar('-'));
//
class ByChar {
public:
...
//   std::vector<std::string> v = bela::StrSplit(L"a,b=c", ByAnyChar(L",="));
//   // v[0] == L"a", v[1] == L"b", v[2] == L"c"
//
...
//
//   using bela::ByLength;
//   std::vector<std::string> v = bela::StrSplit(L"123456789", ByLength(3));
...
//
//   using bela::ByLength;
//   std::vector<std::string> v = bela::StrSplit(L"12345", ByLength(2));
//
...
//   std::vector<std::string> v = bela::StrSplit(L"a,b,c", MaxSplits(',', 1));
//
//   // v[0] == L"a", v[1] == L"b,c"
...
// Example:
//
//   std::vector<std::string> v = bela::StrSplit(L",a,,b,", ',', SkipEmpty());
//
//   // v[0] == L"a", v[1] == L"b"
//
...
using EnableSplitIfString =
    typename std::enable_if<std::is_same<T, std::string>::value || std::is_same<T, const std::string>::value,
                            int>::type;
...
// Example:
//
//   std::vector<std::string> v = bela::StrSplit(L"a,b,c,d", ',');
...
//
//   using bela::ByAnyChar;
//   std::vector<std::string> v = bela::StrSplit(L"a,b=c", ByAnyChar(",="));
//   // v[0] == L"a", v[1] == L"b", v[2] == L"c"
//
...
//   std::vector<std::string_view> v = bela::StrSplit(L"a,b,c", ',');
//
//   // Stores results in a std::set<std::string>, which also performs
//   // de-duplication and orders the elements in ascending order.
//   std::set<std::string> a = bela::StrSplit(L"b,a,c,a,b", ',');
//   // v[0] == L"a", v[1] == L"b", v[2] = L"c"
//
...
//   std::map<std::string, std::string> m = bela::StrSplit("a,b,c", ',');
...
//   std::pair<std::string, std::string> p = bela::StrSplit("a,b,c", ',');
...
//   for (std::string_view sp : bela::StrSplit("a=b=c,d=e,f=,g", ',')) {
...
//
//   bela::StrSplit(std::string_view(""), '-');  // {""}
//   bela::StrSplit(std::string_view(), '-');    // {}, but should be {""}
//
// Try not to depend on this distinction because the bug may one day be fixed.
template <typename Delimiter>
strings_internal::Splitter<typename strings_internal::SelectDelimiter<Delimiter>::type, AllowEmpty, std::string_view>
...
strings_internal::Splitter<typename strings_internal::SelectDelimiter<Delimiter>::type, AllowEmpty, std::string>
...
strings_internal::Splitter<typename strings_internal::SelectDelimiter<Delimiter>::type, Predicate, std::string_view>
...
strings_internal::Splitter<typename strings_internal::SelectDelimiter<Delimiter>::type, Predicate, std::string>
